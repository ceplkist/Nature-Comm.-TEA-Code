...
for i=7
    parfor j= [3 4 5]
        labindex = mod(j,worker_num)+1;
        [NAME, cputime, DATA, errorIndicator]  = singleRun(i,j,materials,superstructure,labindex);
        temp_RESULT2(j,:) = {NAME DATA.ConvergenceState cputime DATA errorIndicator};
    end
    for j=[3 4 5]
        RESULT{(i-1)*18+j,1} = temp_RESULT2{j,1};
        RESULT{(i-1)*18+j,2} = temp_RESULT2{j,2};
        RESULT{(i-1)*18+j,3} = temp_RESULT2{j,3};
        RESULT{(i-1)*18+j,4} = temp_RESULT2{j,4};
        RESULT{(i-1)*18+j,5} = temp_RESULT2{j,5};
    end
end
...
for i=16
    parfor j= [17 18]
        labindex = mod(j,worker_num)+1;
        [NAME, cputime, DATA, errorIndicator]  = singleRun(i,j,materials,superstructure,labindex);
        temp_RESULT3(j,:) = {NAME DATA.ConvergenceState cputime DATA errorIndicator};
    end
    for j=[17 18]
        RESULT{(i-1)*18+j,1} = temp_RESULT3{j,1};
        RESULT{(i-1)*18+j,2} = temp_RESULT3{j,2};
        RESULT{(i-1)*18+j,3} = temp_RESULT3{j,3};
        RESULT{(i-1)*18+j,4} = temp_RESULT3{j,4};
        RESULT{(i-1)*18+j,5} = temp_RESULT3{j,5};
    end
end
    save RESULT_CASCADE_optimal.mat RESULT
