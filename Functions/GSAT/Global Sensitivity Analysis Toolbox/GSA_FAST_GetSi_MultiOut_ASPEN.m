...
function [Si,pro] = GSA_FAST_GetSi_MultiOut_ASPEN(pro)
...
N
parfor j=1:N
    [NAME, cputime, DATA, errorIndicator]  = feval(model_function_handle,X(j,:));
    fX_cell{j} = DATA.NPV; % the function output must be a single variable that takes the form of a row vector
    temp_RESULT(j,:) = {NAME DATA.ConvergenceState cputime DATA errorIndicator};
end
for j=1:N
        RESULT{j,1} = temp_RESULT{j,1};
        RESULT{j,2} = temp_RESULT{j,2};
        RESULT{j,3} = temp_RESULT{j,3};
        RESULT{j,4} = temp_RESULT{j,4};
        RESULT{j,5} = temp_RESULT{j,5};
end

save(strjoin(['RESULT_GSA-',RESULT{1,1},'.mat'],''), 'RESULT');
