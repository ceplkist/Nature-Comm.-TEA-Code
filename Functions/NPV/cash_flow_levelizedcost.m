function [obj] = cash_flow_levelizedcost(RESULT, COST, components,x)
...
COST.product.cathode(2) = x(1);
else
end
if length(components.anode) ==2
COST.product.anode(2) = x(2);
else    
end
...
C_O(1) = COST.utility.DWandB * 10 * COST.utility.operating_hour; % Direct wages and benefits (DW&B) 10명
C_O(2) = 0.15*C_O(1); % Direct salaries and benefits	15% of DW&B
C_O(3) = 0.06*C_O(2); % Operating supplies and services		6% of DW&B
C_O(4) = COST.utility.TechAssist2Manufacturing * 10 / COST.utility.shift; %Technical assistance to manufacturing
C_O(5) = COST.utility.ControlLab * 10 / COST.utility.shift; %Control laboratory
